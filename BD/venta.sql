...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-10-2020 a las 20:51:23
...
-- Procedimientos
--
CREATE DEFINER=CURRENT_USER PROCEDURE `actualizar_precio_producto` (IN `n_cantidad` INT, IN `n_precio` DECIMAL(10,2), IN `codigo` INT)  BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `add_detalle_temp` (`codigo` INT, `cantidad` INT, `token_user` VARCHAR(50))  BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `data` ()  BEGIN
...
CREATE DEFINER=CURRENT_USER PROCEDURE `procesar_venta` (IN `cod_usuario` INT, IN `cod_cliente` INT, IN `token` VARCHAR(50))  BEGIN
DECLARE factura INT;
...
(1, 71347267, 'Cliente Frecuente', 965213356, 'Lima - Perú', 2);
...
(1, 71347267, 'Vida Informático', 'Vida Informático', 925491523, 'naju@vidainformatico.com', 'Lima - Perú', '1.18');
...
(4, 'c4ca4238a0b923820dcc509a6f75849b', 1, 1, '1560.00');
