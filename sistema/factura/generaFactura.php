<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		$query = mysqli_query($conexion,"SELECT f.nofactura, DATE_FORMAT(f.fecha, '%d/%m/%Y') as fecha, DATE_FORMAT(f.fecha,'%H:%i:%s') as  hora, f.codcliente, f.estado,
		...
		if($result > 0){
			...
			$query_productos = mysqli_query($conexion,"SELECT p.descripcion,dt.cantidad,dt.observacion,dt.precio_venta,(dt.cantidad * dt.precio_venta) as precio_total
														FROM factura f
